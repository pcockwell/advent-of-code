...
def find_paths(all_caves, path_so_far):
  ...
  small_caves = [label for label in path_so_far if all_caves[label].is_small()]
  path_has_repeated_small_cave = len(set(small_caves)) < len(small_caves)

  current_cave = all_caves[path_so_far[-1]]
  for new_cave in current_cave.linked_caves:
    if new_cave.label == "end":
      solution_path = ','.join(path_so_far + [new_cave.label])
      paths.add(solution_path)
    elif new_cave.label == "start":
      continue
    elif new_cave.is_small() and new_cave.label in path_so_far and path_has_repeated_small_cave:
      continue
    else:
