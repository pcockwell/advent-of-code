def line_coordinates(line):
  ...
  if start_x == end_x:
    ...
    num_points = abs(end_x - start_x) + 1

    subtract_offset_for_x = False
    if end_x < start_x:
      subtract_offset_for_x = True

    subtract_offset_for_y = False
    if end_y < start_y:
      subtract_offset_for_y = True

    for idx in range(num_points):
      if subtract_offset_for_x:
        x_val = start_x - idx
      else:
        x_val = start_x + idx

      if subtract_offset_for_y:
        y_val = start_y - idx
      else:
        y_val = start_y + idx
